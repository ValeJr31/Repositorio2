	...
	if(!buscarNodo(arbol, n)){
		...
		if(nuevo){
			...
			//arbol no vacio, tiene un nodo o mas
				...
				if(n < valorRaiz)
						insertarNodo(arbol->izd, n, arbol); //insercion a la izquierda
				else
						insertarNodo(arbol->der, n, arbol); //insercion a la derecha
			}
		}
		else
...
void eliminarNodo(Nodo *nodoEliminar){
	...
	else if(nodoEliminar->der){  //cuando el nodo tiene 1 hijo por derecho
...
Nodo *minimo(Nodo *arbol){
		...
		if(arbol->izd)  //si tiene hijo izquierdo
				return minimo(arbol->izd);  //se busca el nodo mas a la izquierda
		else   //si no tiene hijo izquierdo
				return arbol;  //se retima el mismo nodo
}
